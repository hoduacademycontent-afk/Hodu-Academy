...
export default function FacultyApplicationsPage() {
  ...
  async function saveNotificationEmail() {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        ...
        setEmailStatus({ loading: false, message: 'Recipient email updated successfully! All future enquiries & applications will go here.', isError: false })
      }
    } catch (err: any) {
  ...
  async function sendTestEmailFromFaculty() {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        ...
        if (detailApp && detailApp.id === id) {
          setDetailApp((curr: any) => ({ ...curr, status: newStatus }))
        }
      }
    } catch (err) {
  ...
  async function saveDetailNotes() {
    ...
    try {
      const res = await fetch('/api/faculty-apply', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: detailApp.id, notes: internalNotes })
      })
      const data = await res.json()
      if (res.ok && data.success) {
        ...
        setDetailApp((curr: any) => ({ ...curr, notes: internalNotes }))
      } else {
  ...
  return (
    <AdminLayout>
      <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-neutral-200 shadow-xs">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              className="flex items-center gap-1.5 border border-[#F3DCDC] hover:border-[#7E0D0D] text-[#1B2A44] text-xs font-semibold px-3.5 py-2.5 rounded-xl transition-colors bg-white shadow-xs cursor-pointer"
              title="Change Faculty Application & Lead Notification Recipient Email"
            >
        ...
        {/* Lead & Faculty Notification Settings Modal */}
        {showEmailModal && (
          <Modal title="Lead & Faculty Notification Settings" onClose={() => setShowEmailModal(false)}>
            <div className="space-y-4">
              <div className="bg-[#FDF5F5] border border-[#F3DCDC] rounded-xl p-4">
                <p className="text-xs text-[#1B2A44] leading-relaxed">
                  All admission enquiries, student consultation requests, and <strong>teacher/faculty applications</strong> will dispatch instant HTML alerts to this email address via <strong>Resend</strong>.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    onClick={sendTestEmailFromFaculty}
                    disabled={emailStatus?.loading}
                    className="flex items-center gap-1.5 bg-[#FDF5F5] hover:bg-[#7E0D0D] hover:text-white text-[#7E0D0D] border border-[#F3DCDC] text-xs font-bold px-3.5 py-2.5 rounded-xl transition-colors disabled:opacity-50 shrink-0 cursor-pointer"
                  >
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 border border-[#F3DCDC] text-[#1B2A44] py-2.5 rounded-xl text-sm font-semibold hover:bg-neutral-50 transition-colors cursor-pointer"
                >
                  Close
                </button>
                <button
                  ...
                  disabled={emailStatus?.loading}
                  className="flex-1 bg-[#7E0D0D] text-white py-2.5 rounded-xl text-sm font-bold shadow-xs hover:bg-[#600a0a] transition-colors disabled:opacity-60 cursor-pointer"
                >
