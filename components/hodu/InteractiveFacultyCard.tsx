...
export default function InteractiveFacultyCard({
  ...
  const cleanFirstName = faculty.name.replace(/^(Mr\.|Ms\.|Mrs\.|Dr\.|Prof\.|Miss)\s+/i, '').trim().split(' ')[0] || faculty.name

  const modalContent = showModal && mounted ? (
    <div 
      className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200 overflow-y-auto"
      onClick={() => setShowModal(false)}
      style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh' }}
    >
      <div
        className="relative w-full max-w-lg bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border-2 border-[#921e1f]/30 my-auto max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 text-left"
      ...
      >
        ...
        <button
          type="button"
          onClick={() => setShowModal(false)}
          className="absolute top-4 right-4 p-2 rounded-full bg-neutral-100 hover:bg-neutral-200 text-neutral-600 hover:text-neutral-900 transition-all cursor-pointer z-10"
        >
          <X size={18} />
        </button>
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 pb-5 border-b border-neutral-100">
          ...
          <div className="text-center sm:text-left flex-1 min-w-0">
            ...
            <h3 className="font-serif-editorial text-2xl font-bold text-neutral-900 leading-tight">
              {faculty.name}
            </h3>
        ...
        <div className="grid grid-cols-2 gap-3 my-4">
          {faculty.qualification && (
            <div className="bg-neutral-50 border border-neutral-200/80 rounded-xl p-3">
              ...
              <span className="text-xs font-bold text-neutral-800 line-clamp-2">
                {faculty.qualification}
              </span>
            </div>
          )}
        ...
        <div className="mt-6 pt-4 border-t border-neutral-100 flex flex-col sm:flex-row items-center justify-between gap-3">
          <span className="text-xs text-neutral-500 font-medium text-center sm:text-left">
            Want to study with {cleanFirstName}?
          </span>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
            <Link
              href="/contact"
              onClick={() => setShowModal(false)}
              className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 bg-[#921e1f] hover:bg-[#651416] text-white text-xs font-bold px-5 py-2.5 rounded-xl transition-all shadow-sm active:scale-95 cursor-pointer whitespace-nowrap"
            >
              <Calendar size={14} />
            ...
            <button
              type="button"
              onClick={() => setShowModal(false)}
              className="inline-flex items-center justify-center px-4 py-2.5 rounded-xl border border-neutral-200 text-neutral-700 hover:bg-neutral-50 text-xs font-semibold transition-colors cursor-pointer shrink-0"
            >
              Close
            </button>
          </div>
  ...
  return (
    <>
      ...
      {/* Render Modal into document.body to escape transformed container parents */}
